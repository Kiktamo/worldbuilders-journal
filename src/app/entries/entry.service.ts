...
export class EntryService {
  ...
  getEntriesByCategory(category: string): Observable<Entry[]> {
    return this.http.get<Entry[]>(`${this.apiUrl}/category/${category}`);
  }

  getRecentEntries(limit: number = 5): Observable<Entry[]> {
    return this.http.get<Entry[]>(`${this.apiUrl}/recent/${limit}`);
  }
}